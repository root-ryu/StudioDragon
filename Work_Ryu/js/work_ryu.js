    ...
    for (let i = 0; i < count; i++) {
        ...
        if (replyCount > 0) {
            ...
            for (let j = 0; j < replyCount; j++) {
                ...
                const replyElement = createReplyElement(replyText, replyTimestamp);
...
function createReplyElement(text, timestamp) {
    const isMale = random.boolean();
    const name = random.item(isMale ? DATA.maleNames : DATA.femaleNames);
    const bgColor = random.item(ASSETS.profileBg);
    const timeDiff = getTimeDifference(timestamp);
...
function addUserReply(repliesContainer, replyText, btn) {
    initializeUser();
    const timestamp = Date.now();
    const newReply = createReplyElement(replyText, timestamp);
    const replyInputWrapper = repliesContainer.querySelector('.reply_input_wrapper');
...
document.querySelectorAll('.comment_item').forEach(comment => {

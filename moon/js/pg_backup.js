...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (gallery) {
    ...
    const getGalleryOptions = (container) => {
      if (container.classList.contains('popular_img_slide')) {
      ...
      if (container.classList.contains('new_img_slide')) {
        const pagination = container.parentElement.querySelector('.swiper-pagination');
        return {
          effect: 'slide',
          loop: true,
      ...
      if (container.classList.contains('popular_video_slide')) {
        const pagination = container.parentElement.querySelector('.swiper-pagination');
        return {
          effect: 'coverflow',
          loop: true,
    ...
    const initOrRefreshSwiper = (container) => {
      if (!container) return;
      if (!container.swiper) {
        const opts = getGalleryOptions(container);
        if (opts) {
        ...
        swiper.update();
        // centeredSlides인 경우 초기 슬라이드(초기 설정값)로 이동해 좌측 여백 이슈 방지
        if (swiper.params.centeredSlides) {
          const initIdx = typeof swiper.params.initialSlide === 'number' ? swiper.params.initialSlide : 0;
          if (swiper.params.loop) {
            swiper.slideToLoop(initIdx, 0, false);
          } else {
            swiper.slideTo(initIdx, 0, false);
          }
        } else {
          // 좌측 정렬 슬라이더는 0번으로 리셋
    ...
    categoryBtns.forEach((btn, index) => {
      btn.addEventListener("click", () => {
        ...
        if (index === 0) {
          ...
          setTimeout(() => {
            fanarts.querySelectorAll('.popular_img_slide, .new_img_slide').forEach(initOrRefreshSwiper);
          }, 0);
        } else {
  ...
  */
  ...
  if (popularVideoContainer && popularVideoContainer.offsetParent !== null) {
  ...
  if (newVideoContainer && newVideoContainer.offsetParent !== null) {
